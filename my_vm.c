...
void a_free(void *va, uint64_t size) {
	...
	if(free_flag) {
		...
		for(pageno_t vpn_i=vpn;vpn_i<vpn+num_pages;++vpn_i) {
			pdindex = vpn_i>>LEVELBITS;
			ptindex = vpn_i & ~((~0)<<LEVELBITS);
...
void put_value(void *va, void *val, int size) {
	...
	if(vpn_start == vpn_end) {
		...
		pa = translate((address_t)(vpn_end<<pagesizebits));
		memcpy((void*)pa, val, size);
	}
}
